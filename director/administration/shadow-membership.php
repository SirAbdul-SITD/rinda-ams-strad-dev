...
if (isset($_POST['shadow_id'], $_POST['student_id'], $_POST['amount_fare'])) {

  $shadow_id = $_POST['shadow_id'];
  ...
  $query = "SELECT status 
            FROM _members 
            WHERE shadow_id = :shadow_id AND student_id = :student_id AND session = :session AND term = :term";
  $stmt = $pdo->prepare($query);
  $stmt->bindParam(':shadow_id', $shadow_id);
  ...
  if (count($member_available) > 0) {
    ...
    $insertQuery = "INSERT INTO `_members` (`shadow_id`, `parent_id`, `student_id`, `fee`, session, term) VALUES (:shadow_id, :parent_id, :student_id, :fee, :session, :term)";
    $insertQ = $pdo->prepare($insertQuery);
    $insertQ->bindParam(':shadow_id', $shadow_id, PDO::PARAM_INT);
...
<head>
  ...
  <title>Shadow Subscribers | Grithal Academy</title>
  <!-- Simple bar CSS -->
...
<body class="vertical  light  ">
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
          ...
          <ul class="navbar-nav flex-fill w-100 mb-2">
            ...
            <li class="nav-item active">
              <a class="nav-link text-primary" href="shadow.php">
            ...
            <div class="row">
              ...
              <div class="col-md-12 my-4">
                <div class="row align-items-center my-3">
                  <div class="col">
                    <h3 class="page-title">Shadow Subscribers</h3>
                  </div>
                  <div class="col-auto">
                    <a href="shadow.php">
                      <button type="button" class="btn btn-primary"><span class="fe fe-plus fe-16 mr-3"></span>Manage Shadow Categories</button></a>
                  </div>
                </div>
                  ...
                  <div class="card-body">
                    ...
                    <table class="table table-borderless table-hover">
                      ...
                      <tbody>
                        ...
                        $query = "SELECT s.id AS student_id, s.admission_no, CONCAT(s.firstName, ' ', s.lastName) AS full_name, m.student_id, m.parent_id, m.shadow_id, m.status, m.fee, m.id AS fee_id, m.paid_amount, m.payment_date, r.id, m.duration, r.amount FROM students s INNER JOIN shadow_members m ON m.student_id = s.id INNER JOIN shadow_fees r ON r.id = m.shadow_id WHERE s.status = 1 AND m.session = :session AND m.term = :term";
                        $stmt = $pdo->prepare($query);
                        $stmt->bindParam(':session', $curr_session);
                        $stmt->bindParam(':term', $term);
                        $stmt->execute();
                        $shadows = $stmt->fetchAll(PDO::FETCH_ASSOC);



                        if (count($shadows) > 0) {
                          // Loop through each shadow and display it in the table
                          foreach ($shadows as $index => $shadow) {
                            echo "<tr>";
                            $l_fee_id = $shadow['fee_id'];
                            $l_student_id = $shadow['student_id'];
                            $l_status = $shadow['status'];

                            echo "<td>" . ($index + 1) . "</td>";
                            echo "<td>" . $shadow['full_name'] . "</td>";

                            if ($shadow['duration'] == '1') {
                              echo "<td>A Term</td>";
                            } else {
                              echo "<td>Whole Session</td>";
                            }

                            if ($shadow['duration'] == '1') {
                              $formatted_amount = '₦ ' . number_format($shadow['amount'], 2);
                            } else {
                              $term = $shadow['amount'] * 3;
                              $formatted_amount = '₦ ' . number_format($term, 2);
                            }
                            ...
                            if ($shadow['payment_date'] == '0000-00-00') {
                              echo "<td>Not started</td>";
                            } else {
                              echo "<td>" . $shadow['payment_date'] . "</td>";
                            }


                            if ($shadow['payment_date'] == '0000-00-00') {
                              echo "<td>Not started</td>";
                            } elseif ($shadow['duration'] == '1') {
                              echo "<td>End of term</td>";
                            } elseif ($shadow['duration'] == '2') {
                              echo "<td>End of session</td>";
                            }
                            ...
                            $admission_no = $shadow['admission_no'];
                            $parent_id = $shadow['parent_id'];
                            echo "<td class='dropOptions'>
                            <input type='hidden' value='$l_fee_id' class='shadow-id' />
                            <input type='hidden' value='$l_student_id' class='student-id' />
                            <input type='hidden' value='$l_status' class='shadow-status' />
                          ...
                          // If no shadows are found, display a message
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('.table').on('click', '.p-status', function() {
        // $('').on('click'

        var row = $(this).closest('tr');
        var shadow_Id = row.find('.shadow-id').val();
        var student_Id = row.find('.student-id').val();
        var shadow_status = row.find('.shadow-status').val();


        var l_status = (shadow_status == 'Paid') ? 'Unpaid' : 'Paid';


        // Update form fields with retrieved data
        $('#update-id').val(shadow_Id);
        $('#update-student-id').val(student_Id)
        $('#update-status').val(l_status);
        $('#prev-status').text(shadow_status);
