...
if (isset($_POST['admission_no']) || isset($_GET['admission_no'])) {
  $admission_no = $_POST['admission_no'] ?? $_GET['admission_no'];
} else {
...
<head>
  ...
  <title>Student Profile - Academics | Rinda AMS</title>
  <!-- Simple bar CSS -->
...
<body class="vertical  light  ">
                ...
                <div class="card-body my-4">
                  <div class="row align-items-center">
                    ...
                    if ($student) {
                      ...
                      {
                        global $pdo;
                        $query = "SELECT session FROM sessions WHERE id = :sessionId";
                        $stmt = $pdo->prepare($query);
                        $stmt->bindParam(':sessionId', $sessionId, PDO::PARAM_INT);
                        $stmt->execute();
                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                        return ($result) ? $result['session'] : "Unknown Session";
                      }
                      ...
                      <div class="col-md-3 text-center mb-5">
                        <a href="#!" class="avatar avatar-xl">
                          <?php if (!empty($student['photo'])) { ?>
                            <img src="../../uploads/student-profiles/<?= $student['photo'] ?>" alt="..." class="avatar-img rounded-circle">
                          <?php } else { ?>
                            <img src="../../uploads/student-profiles/1.png" alt="..." class="avatar-img rounded-circle">
                          <?php } ?>
                      ...
                      <div class="col">
                        <div class="row align-items-center">
                          <div class="col-md-7">
                            <h4 class="mb-1 student-name">
                              <?= displayIfEmpty($student['firstName'] . ' ' . $student['lastName']); ?>
                            </h4>
                            <p class="mb-3"><span class="badge badge-dark student-grade">
                                <?= displayIfEmpty($student['admission_no']); ?>
                              </span></p>
                          </div>
                          <div class="col">
                          </div>
                          ...
                          <div class="col">
                            <div class="row">
                              ...
                              <p class="student-email">
                                <?= displayIfEmpty($student['email']); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">Phone Number: </p>
                              <p class="student-phone">
                                <?= displayIfEmpty($student['phoneNumber']); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">Admitted Term: </p>
                              <p class="student-admitted-term">
                                <?= displayIfEmpty(getTermName($student['term'])); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">Admitted Session: </p>
                              <p class="student-admitted-session">
                                <?= displayIfEmpty(getSessionName($student['session'])); ?>
                              </p>
                            </div>
                            ...
                            <div class="row">
                              ...
                              <p class="student-address">
                                <?= displayIfEmpty($student['address']); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">City: </p>
                              <p class="student-city">
                                <?= displayIfEmpty($student['city']); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">State: </p>
                              <p class="student-state">
                                <?= displayIfEmpty($student['state']); ?>
                              </p>
                            </div>
                            <div class="row">
                              <p class="text-muted">Country: </p>
                              <p class="student-country">
                                <?= displayIfEmpty($student['country']); ?>
                              </p>
                            </div>
                          </div>
                        </div>
                        <div class="row ">
                          <div class="col-md-4 mb-2">
                            <span class=" mb-0 ">Admission Date <span id="student-joined">
                                <?= displayIfEmpty($student['join_date']); ?>
                              </span></span>
                          </div>
                          <div class="row mb-2">
                            <form class="m-1" action="edit_student.php" method="post">
                              <input type="hidden" name="admission_no" value="<?php echo $admission_no; ?>">
                              <button type="submit" class="btn mb-2 btn-outline-primary">Edit Profile</button>
                            </form>
                            <form class="m-1" action="parents.php" method="post">
                              <input type="hidden" name="id" value="<?php echo $student['id']; ?>">
                              <button type="submit" class="btn mb-2 btn-outline-primary">Manage Parents</button>
                            </form>
                            <form class="m-1" action="class_management.php" method="post">
                              <input type="hidden" name="id" value="<?= $student['id'] ?>">
                              <button type="submit" class="btn mb-2 btn-outline-primary">Manage Class</button>
                            </form>

                            <?php if ($student['status'] == 1) { ?>
                              <form class="m-1" id="change-student-status" method="post">
                                <input type="hidden" name="studentId" value="<?= $student['id']; ?>">
                                <input type="hidden" name="status" value="0">
                                <button type="submit" class="change-status btn mb-2 btn-outline-danger change-status">Disable Student</button>
                              </form>
                            ...
                            <?php } else { ?>
                              <form class="m-1" id="change-student-status" method="post">
                                ...
                                <button type="submit" class="change-status btn mb-2 btn-outline-success">Enable Student</button>
                              </form>

                            <?php } ?>

                            <!-- <button type="button" class="btn mb-2 btn-outline-secondary"></button>
                            <button type="button" class="btn mb-2 btn-outline-success">Manage Class</button>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-danger confirm-remove">Change Status</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      const removeButtons = document.querySelectorAll('.change-status');
        ...
        button.addEventListener('click', function(event) {
          ...
          $('.confirm-remove').off('click').on('click', function() {
            ...
            $.ajax({
              ...
              data: $('#change-student-status').serialize(),
              dataType: 'json',
              success: function(response) {
                if (response.success) {
