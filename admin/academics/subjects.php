              ...
              <div class="col-md-12">
                <div class="card shadow">
                  <div class="card-body">
                    ...
                    WHERE s.status = 1 AND c.status = 1 AND s.assigned = :user
                        ...
                        <tbody>
                          ...
                          foreach ($subjects as $index => $subject) : ?>
                            <tr data-id="<?= $subject['id']; ?>">
                              ...
                              <td>
                                <?php
                                $assessing_class = $subject['class_id'];
                                // Fetch assessment types
                                $query_assessment_count = "SELECT * FROM assessment_types WHERE class_id = :class ORDER BY `assessment_types`.`assessment_id` ASC";
                                $stmt_assessment_count = $pdo->prepare($query_assessment_count);
                                $stmt_assessment_count->bindParam(':class', $assessing_class, PDO::PARAM_INT);
                                $stmt_assessment_count->execute();
                                $assessment_counts = $stmt_assessment_count->fetchAll(PDO::FETCH_ASSOC);

                                echo count($assessment_counts) . ' assessmensts';
                                ?>
                              ...
                              <td>
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                  ...
                                  <form action="../targets/subject-target.php" method="post">
                                    <input type="hidden" name="subject" value="<?= $subject['id']; ?>">
                                    <a class="dropdown-item" type="submit">Targets</a>
                                  </form>

                                  <form action="../assessments/subject-assessment.php" method="post">
                                    <input type="hidden" name="subject" value="<?= $subject['id']; ?>">
                                    <button class="dropdown-item" type="submit">Assessments</button>
                                  </form>
