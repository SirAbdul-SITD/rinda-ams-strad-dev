...
if (!empty($search)) {
  // Use CONCAT to combine first_name and last_name into full_name
  $where = " WHERE 
        CONCAT(s.first_name, ' ', s.last_name) LIKE :search OR 
        s.email LIKE :search OR 
        s.mobile_number LIKE :search OR 
        s.marital_status LIKE :search";
  $params['search'] = "%$search%";
}

// Add the designation_id filter. If there are already other conditions, use 'AND' otherwise just start the WHERE clause.
$where .= !empty($where) ? " AND s.designation_id != 1" : " WHERE s.designation_id != 1";

// Count total for pagination
$countSQL = "SELECT COUNT(DISTINCT s.id) $baseSQL $where";
...
$dataSQL = "
    SELECT 
        s.id, CONCAT(s.first_name, ' ', s.last_name) AS full_name, s.marital_status, s.email, s.mobile_number
    $baseSQL
    $where
...
<head>
  ...
  <title>Staff Contacts - Messages | Rinda AMS</title>
  <!-- Simple bar CSS -->
        ...
        <div class="row justify-content-center">
          <div class="col-12">
            <div class="row align-items-center my-4">
              <div class="col">
                <h2 class="h3 mb-0 page-title">Staff Contacts</h2>
              </div>
              <!-- Search Bar -->
                  ...
                  <div class="card shadow mb-4">
                    <div class="card-body text-center">
                      ...
                      <div class="card-text my-2">
                        <strong class="card-title my-0"><?= htmlspecialchars($staff['full_name']) ?></strong>
                        <p class="small text-muted mb-0"><?= htmlspecialchars($staff['mobile_number']) ?></p>
                        <p class="small"><span
                            class="badge badge-light text-muted"><?= htmlspecialchars($staff['email']) ?></span>
                        </p>
                      </div>
                    </div> <!-- ./card-text -->
                    <div class="card-footer">
                      <div class="row align-items-center justify-content-between">
                        <div class="col-auto">
                          <small>
                            <?php
                            $maritalStatus = htmlspecialchars($staff['marital_status']);

                            // Determine the background class and text based on marital status
                            if ($maritalStatus === 'Married') {
                              ...
                              $statusText = 'Not disclosed';
                            }
                            ?>

                            <span class="dot dot-lg <?= $bgClass ?> mr-1"></span>
                            <?= $statusText ?>
                          </small>
                          ...
                          <div class="file-action">
                            ...
                            <div class="dropdown-menu m-2">
                              <form action="../academics/parent.php" method="post" target="_blank">
                                <input type="hidden" name="parent_id" value="<?= $staff['id'] ?>">
                              ...
                              <form action="compile-notification.php" method="post" target="_blank">
                                <input type="hidden" name="x_id" value="<?= $staff['id'] ?>">
