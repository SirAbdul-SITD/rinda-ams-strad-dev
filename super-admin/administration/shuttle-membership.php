...
if (isset($_POST['shuttle_id'], $_POST['student_id'], $_POST['amount_fare'])) {
  ...
  $query = "SELECT status 
            FROM shuttle_members 
            WHERE shuttle_id = :shuttle_id AND student_id = :student_id AND session = :session AND term = :term";
  ...
  if (count($member_available) > 0) {
    ...
    $insertQuery = "INSERT INTO `shuttle_members` (`shuttle_id`, `parent_id`, `student_id`, `fee`, session, term) VALUES (:shuttle_id, :parent_id, :student_id, :fee, :session, :term)";
    $insertQ = $pdo->prepare($insertQuery);
    $insertQ->bindParam(':shuttle_id', $shuttle_id, PDO::PARAM_INT);
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
          ...
          <ul class="navbar-nav flex-fill w-100 mb-2">
            ...
            <li class="nav-item active">
              <a class="nav-link text-primary" href="shuttle.php">
            ...
            <div class="row">
              ...
              <div class="col-md-12 my-4">
                <div class="row align-items-center my-3">
                  <div class="col">
                    <h3 class="page-title">School Shuttle Subscribers</h3>
                  </div>
                  <div class="col-auto">
                    <a href="shuttle.php">
                      <button type="button" class="btn btn-primary"><span class="fe fe-plus fe-16 mr-3"></span>Manage Shuttle Categories</button></a>
                  </div>
                </div>
                  ...
                  <div class="card-body">
                    ...
                    <table class="table table-borderless table-hover">
                      ...
                      <tbody>
                        ...
                        $query = "SELECT s.admission_no, CONCAT(s.firstName, ' ', s.lastName) AS full_name, m.id, m.student_id, m.parent_id, m.route_id, m.status, m.paid_amount, m.payment_date, r.route, r.type, r.fare, r.category FROM students s INNER JOIN shuttle_members m ON m.student_id = s.id INNER JOIN routes r ON r.id = m.route_id WHERE s.status = 1 AND m.session = :session AND m.term = :term";
                        $stmt = $pdo->prepare($query);
                        $stmt->bindParam(':session', $curr_session);
                        $stmt->bindParam(':term', $term);
                        $stmt->execute();
                        ...
                        if (count($shuttles) > 0) {
                          // Loop through each shuttle and display it in the table
                          foreach ($shuttles as $index => $shuttle) {
                            echo "<tr>";
                            $l_membership_id = $shuttle['id'];
                            $l_student_id = $shuttle['student_id'];
                            $l_status = $shuttle['status'];

                            echo "<td>" . ($index + 1) . "</td>";
                            echo "<td>" . $shuttle['full_name'] . "</td>";

                            echo "<td>" . $shuttle['route'] . "</td>";

                            echo "<td>" . $shuttle['type'] . "</td>";

                            echo "<td>" . $shuttle['category'] . "</td>";


                            $formatted_amount = '₦ ' . number_format($shuttle['fare'], 2);
                            echo "<td>" . $formatted_amount . "</td>";


                            if ($shuttle['payment_date'] == '0000-00-00') {
                              echo "<td>Not Paid</td>";
                            } else {
                              echo "<td>" . $shuttle['payment_date'] . "</td>";
                            }
                            ...
                            $admission_no = $shuttle['admission_no'];
                            $parent_id = $shuttle['parent_id'];
                            echo "<td>
                             <input type='hidden' value='$l_membership_id' class='shuttle-id' ?>
                            <input type='hidden' value='$l_status' class='shuttle-status' ?>
                          ...
                          // If no shuttles are found, display a message
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form id="updateForm">
              ...
              <input type="hidden" id="update-shuttle-id" name="shuttle-id" value="">
            </form>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('.table').on('click', '.dropdown-item.p-status', function() {
        ...
        var shuttle_Id = row.find('.shuttle-id').val();
        var shuttle_status = row.find('.shuttle-status').val();
        var student_id = row.find('.student-id').val();

        if (shuttle_status == 'Paid') {
          l_status = 'Unpaid'
        } else {
          l_status = 'Paid'
        }
        ...
        $('#update-shuttle-id').val(shuttle_Id);
        $('#update-status').val(l_status);
        $('#prev-status').text(shuttle_status);
    ...
    $('.confirm-update').on('click', function() {
      ...
      $.ajax({
        url: 'update-shuttle-payment.php',
        type: 'POST',
        data: form.serialize(),
