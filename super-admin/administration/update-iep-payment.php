...
if (isset($_POST['id']) && isset($_POST['status'])) {
    ...
    $query = "SELECT m.status, m.parent_id, m.iep_id, i.id FROM iep_members m INNER JOIN iep_fees i ON i.id = m.iep_id WHERE m.id = :id";
    ...
    if (!$exist) {
        ...
        $updateQuery = "UPDATE `iep_members` SET `status` = :status, `payment_date` = :date WHERE `id` = :id";
        ...
        if ($updateQ->execute()) {
            if (!empty($parent_id)) {
                ...
                if (!empty($parent['email'])) {
                    ...
                    $comment = '';
                    if ($status == 'Unpaid') {
                        $comment = "We appreciate your trust in Grithall Academy's student iep services. An invoice for your child/ward's iep subscription is currently unpaid. Kindly login to your dashboard to view the status of your subscriptions and ensure to complete payments in due time to avoid payment default.";
                    } else {
                        $comment = "We appreciate your trust in Grithall Academy's student iep services. We have received payment for your child's iep subscription. Kindly login to your dashboard to view the status of your subscriptions.";
                    }
