...
if (isset($_POST['iep_id'], $_POST['student_id'], $_POST['amount_fare'])) {
  ...
  $query = "SELECT status 
            FROM _members 
            WHERE iep_id = :iep_id AND student_id = :student_id AND session = :session AND term = :term";
  $stmt = $pdo->prepare($query);
  $stmt->bindParam(':iep_id', $iep_id);
  ...
  if (count($member_available) > 0) {
    ...
    $insertQuery = "INSERT INTO `_members` (`iep_id`, `parent_id`, `student_id`, `fee`, session, term) VALUES (:iep_id, :parent_id, :student_id, :fee, :session, :term)";
    $insertQ = $pdo->prepare($insertQuery);
    $insertQ->bindParam(':iep_id', $iep_id, PDO::PARAM_INT);
...
<head>
  ...
  <title>IEP Subscribers | Grithal Academy</title>
  <!-- Simple bar CSS -->
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
          ...
          <ul class="navbar-nav flex-fill w-100 mb-2">
            ...
            <li class="nav-item active">
              <a class="nav-link text-primary" href="iep.php">
            ...
            <div class="row">
              ...
              <div class="col-md-12 my-4">
                <div class="row align-items-center my-3">
                  <div class="col">
                    <h3 class="page-title">IEP Subscribers</h3>
                  </div>
                  <div class="col-auto">
                    <a href="iep.php">
                      <button type="button" class="btn btn-primary"><span class="fe fe-plus fe-16 mr-3"></span>Manage IEP Categories</button></a>
                  </div>
                </div>
                  ...
                  <div class="card-body">
                    ...
                    <table class="table table-borderless table-hover">
                      ...
                      <tbody>
                        ...
                        $query = "SELECT s.id AS student_id, s.admission_no, CONCAT(s.firstName, ' ', s.lastName) AS full_name, m.student_id, m.parent_id, m.iep_id, m.status, m.fee, m.id AS fee_id, m.paid_amount, m.payment_date, r.id, m.duration, r.amount FROM students s INNER JOIN iep_members m ON m.student_id = s.id INNER JOIN iep_fees r ON r.id = m.iep_id WHERE s.status = 1 AND m.session = :session AND m.term = :term";
                        $stmt = $pdo->prepare($query);
                        $stmt->bindParam(':session', $curr_session);
                        $stmt->bindParam(':term', $term);
                        $stmt->execute();
                        ...
                        if (count($ieps) > 0) {
                          // Loop through each iep and display it in the table
                          foreach ($ieps as $index => $iep) {
                            echo "<tr>";
                            $l_fee_id = $iep['fee_id'];
                            $l_student_id = $iep['student_id'];
                            $l_status = $iep['status'];

                            echo "<td>" . ($index + 1) . "</td>";
                            echo "<td>" . $iep['full_name'] . "</td>";
                            ...
                            $formatted_amount = '₦ ' . number_format($iep['fee'], 2);
                            echo "<td>" . $formatted_amount . "</td>";

                            if ($iep['payment_date'] == '0000-00-00') {
                              echo "<td>Not started</td>";
                            } else {
                              echo "<td>" . $iep['payment_date'] . "</td>";
                            }


                            if ($iep['payment_date'] == '0000-00-00') {
                              echo "<td>Not started</td>";
                            } elseif ($iep['duration'] == '1') {
                              echo "<td>End of term</td>";
                            } elseif ($iep['duration'] == '2') {
                            ...
                            $parent_id = $iep['parent_id'];
                            echo "<td class='dropOptions'>
                            <input type='hidden' value='$l_fee_id' class='iep-id' />
                            <input type='hidden' value='$l_student_id' class='student-id' />
                            <input type='hidden' value='$l_status' class='iep-status' />
                          ...
                          // If no ieps are found, display a message
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('.table').on('click', '.p-status', function() {
        ...
        var iep_Id = row.find('.iep-id').val();
        var student_Id = row.find('.student-id').val();
        var iep_status = row.find('.iep-status').val();


        var l_status = (iep_status == 'Paid') ? 'Unpaid' : 'Paid';


        // Update form fields with retrieved data
        $('#update-id').val(iep_Id);
        $('#update-student-id').val(student_Id)
        $('#update-status').val(l_status);
        $('#prev-status').text(iep_status);
