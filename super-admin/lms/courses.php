                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="table table-hover" id="coursesTable">
                        ...
                        <tbody>
                          <?php
                          // Get all courses with curriculum type name
                          try {
                            $stmt = $pdo->query("
                              SELECT c.course_id, c.course_name, c.course_code, 
                                     ct.name as curriculum_type, c.level, c.thumbnail,
                                     c.created_at
                              FROM courses c
                            ...
                            if (empty($courses)) {
                              ...
                              foreach ($courses as $course) {
                                ...
                                $thumbnail = $course['thumbnail'] ? './' . $course['thumbnail'] : '../assets/images/default-course.jpg';
                                echo '
                                <tr>
                                  <td>' . htmlspecialchars($course['course_id']) . '</td>
                                  <td><img style="max-width: 70px; max-height: 70px; border-radius: 5px;" src="' . $thumbnail . '" class="course-thumbnail" alt="Course thumbnail"></td>
                                  <td>' . htmlspecialchars($course['course_name']) . '</td>
                                  <td>' . htmlspecialchars($course['course_code']) . '</td>
                                  <td>' . htmlspecialchars($course['curriculum_type']) . '</td>
                                  <td>' . htmlspecialchars($course['level']) . '</td>
                                  <td>' . $created_at . '</td>
                                  <td class="action-buttons">
                                    <a href="course-details.php?id=' . $course['course_id'] . '" class="btn btn-sm btn-outline-primary" title="View">
                                      <i class="fe fe-eye"></i>
                                    </a>
                                    <button class="btn btn-sm btn-outline-warning edit-course" 
                                            data-id="' . $course['course_id'] . '"
                                            data-name="' . htmlspecialchars($course['course_name']) . '"
                                            data-code="' . htmlspecialchars($course['course_code']) . '"
                                            data-type="' . htmlspecialchars($course['curriculum_type']) . '"
                                            data-type-id="' . htmlspecialchars($course['curriculum_type_id'] ?? '') . '"
                            ...
                            echo '<tr><td colspan="8" class="text-center text-muted">Unable to load courses.</td></tr>';
                          }
                          ?>
                        </tbody>
                      </table>
  ...
  <script>
    $(document).ready(function () {
      ...
      $('.edit-course').click(function () {
        ...
        var curriculumTypeId = $(this).data('type-id');
