...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($vidId && $newTitle && $newClass && $newSubject) {
        // Prepare and execute the SQL statement to update Audio information
        ...
        if ($stmt->execute()) {
            ...
            $response = [
                'status' => 'success',
                'message' => 'Audio information updated successfully.'
            ];
            ...
            $response = [
                'status' => 'error',
                'message' => 'Failed to update Audio information. Please try again.'
            ];
            echo json_encode($response);
            exit();
        }
